...
export class XrayService {
  ...
  async createRun(results: XrayTestResult, execInfo: ExecInfo) {
    ...
    try {
      ...
      if (this.dryRun) {
        logger.sucess("😀 Successfully performed a Dry Run");
        //logger.console.log(`${bold(green("😀 Successfully performed a Dry Run"))}`);
      } else {
        logger.sucess("😀 Successfully sending test results to Jira");
      ...
      try {
        ...
        logger.error(`Unable to write playwright-xray-error.log : ${(error as Error).message}`);
        //console.log(`Unable to write playwright-xray-error.log : ${(error as Error).message}`);
      }

      const msgs = msg.split(";");
      logger.error("😞 Error sending test results to Jira");
      ...
      logger.logEmptyLine();
      //console.log(`${bold(blue(" "))}`);
      logger.info("👉 Check the 'playwright-xray-error.log' file for more details");
    ...
    function writeRunResult(testPlan: string) {
      ...
      try {
        ...
        logger.error(`Unable to write runresult.json : ${(error as Error).message}`);
